...
import { Heart, MessageCircle, Camera, Send, MapPin, X, Image as ImageIcon, PawPrint } from "lucide-react";
...
export default function FeedPage() {
  const { feedPosts, addFeedPost, likeFeedPost, addFeedComment, myDog } = useStore();
  ...
  const handlePost = () => {
    ...
    addFeedPost(newText, newPhoto, park?.id, park?.name);
    ...
    <div className="flex flex-col h-full w-full bg-[#FAF8F5]">
      ...
      <div className="glass border-b border-white/40 px-4 py-4 flex items-center justify-between flex-shrink-0 shadow-sm z-10">
        <div>
          ...
          <p className="text-[11px] uppercase tracking-wider font-bold text-amber-600">{feedPosts.length} publications</p>
        </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl">
            <div className="p-4 border-b border-[#E2DDD5] flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-sm font-semibold text-[#242019]">{myDog?.name ?? "Mon chien"}</p>
                  <p className="text-xs text-[#7D7269]">{myDog?.breed}</p>
                </div>
              </div>
            ...
            <div className="p-4 space-y-3">
              <textarea value={newText} onChange={(e) => setNewText(e.target.value)}
                placeholder={`Quoi de neuf avec ${myDog?.name ?? "ton chien"} ?`}
                rows={4}
            ...
            <div className="p-4 border-t border-[#E2DDD5] flex items-center justify-between">
              ...
              <button onClick={handlePost} disabled={!newText.trim() && !newPhoto}
          ...
          {feedPosts.map((post) => (
            <article key={post.id} className="glass rounded-3xl border border-white/40 overflow-hidden card-hover group relative">
              ...
              <div className="p-5 pb-3 relative z-10">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-[#242019]">{post.authorDogName}</p>
                      <p className="text-[10px] uppercase tracking-wider font-bold text-[#7D7269]">{post.authorBreed} · {timeAgo(post.timestamp)}</p>
                    </div>
                  </div>
                  {post.parkName && (
              ...
              <div className="px-4 pb-3 flex items-center gap-5 border-t border-[#F2EFE9] pt-3">
                <button onClick={() => likeFeedPost(post.id)}
                  className={`flex items-center gap-1.5 text-sm font-medium transition-colors group ${post.likes.includes("me") ? "text-red-500" : "text-[#7D7269] hover:text-red-400"}`}>
                  <Heart size={17} fill={post.likes.includes("me") ? "currentColor" : "none"} className="group-hover:scale-110 transition-transform" />
                  ...
                  {post.comments.map((c) => (
                    <div key={c.id} className="flex gap-2">
                      ...
                      <div className="bg-[#FAF8F5] rounded-2xl px-3 py-2 flex-1 border border-[#E2DDD5]">
                        <p className="text-xs font-semibold text-[#242019]">{c.authorDogName}</p>
                        <p className="text-xs text-[#7D7269] mt-0.5">{c.text}</p>
                      </div>
                    </div>
                  ))}
                  <div className="flex gap-2">
                    <div className="w-7 h-7 rounded-xl overflow-hidden flex-shrink-0 bg-amber-500 flex items-center justify-center">
                      {myDog?.photo
                        ? <img src={myDog.photo} alt="" className="w-full h-full object-cover" />
